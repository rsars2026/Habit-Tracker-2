...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      --bg: #080808; --bg1: #111111; --bg2: #1a1a1a;
...
const WORKOUTS = [
  ...
  { day: "Tue", label: "STRETCH", emoji: "🧘", exercises: [
    ...
    { name: "Childs Pose", sets: 2, reps: "60s", rest: "30s" },
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle className="ring-fill" cx={size / 2} cy={size / 2} r={r} strokeWidth={stroke} strokeDasharray={circ} strokeDashoffset={offset} />
      </svg>
...
const FoodTab = () => {
  ...
  const addMeal = () => {
    if (!form.name) return;
    setMeals(meals.concat([Object.assign({}, form, { id: Date.now() })]));
    ...
    <div style={{ padding: "52px 20px 100px" }}>
      {sheet && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setSheet(false)}>
          <div className="sheet">
            ...
            {[
              { key: "name", label: "Meal name", placeholder: "e.g. Chicken and rice" },
              ...
              <div key={f.key} style={{ marginBottom: "12px" }}>
                ...
                <input className="field" placeholder={f.placeholder} value={form[f.key]}
                  onChange={e => setForm(Object.assign({}, form, { [f.key]: e.target.value }))} />
              </div>
            ))}
      ...
      {meals.length > 0 && (
        <div style={{ marginBottom: "16px" }}>
          ...
          {meals.map((m, i) => (
            <div key={m.id} style={{ background: "var(--bg1)", border: "1px solid var(--border)", borderRadius: "12px", padding: "14px 16px", marginBottom: "8px", display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ flex: 1 }}>
              ...
              <button onClick={() => removeMeal(m.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "16px", padding: "4px" }}>x</button>
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  const [streak] = useLocal("streak", 0);
  const todayWeight = weightLog.find(e => e.d === todayKey());
  const todayWeightVal = todayWeight ? todayWeight.w : null;
  const toggleHabit = (key) => { setHabits(Object.assign({}, habits, { [key]: !habits[key] })); };
  const logWeight = () => {
    const w = parseFloat(weightInput);
    if (!w) return;
    const existing = weightLog.filter(e => e.d !== todayKey());
    const sorted = existing.concat([{ d: todayKey(), w }]).sort((a, b) => a.d.localeCompare(b.d));
    setWeightLog(sorted);
    setHabits(Object.assign({}, habits, { weight: true }));
    setWeightInput("");
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh", maxWidth: "430px", margin: "0 auto", position: "relative" }}>
      ...
      {weightSheet && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setWeightSheet(false)}>
          <div className="sheet">
            ...
            {todayWeightVal && (
              <div style={{ fontFamily: "var(--mono)", fontSize: "11px", color: "var(--muted)", marginBottom: "16px" }}>Today: {todayWeightVal} kg</div>
            )}
      ...
      <div style={{ overflowY: "auto", height: "100vh" }}>
        {tab === "daily" && <DailyTab habits={habits} toggleHabit={toggleHabit} weight={todayWeightVal} setWeightSheet={setWeightSheet} />}
